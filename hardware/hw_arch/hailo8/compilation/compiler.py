...
class Hailo8Compiler(CompilerBase):
    ...
    async def compile(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            minio = get_minio()
            try:
                await self.log_progress(model_id, "[Hailo8] Descargando modelo base .pt desde MinIO...")
                await minio.fget_object(self._bucket_models, source_key, pt_path)
            except Exception as e:
                logger.error(f"[Hailo8] Failed to download source model: {e}")
            ...
            logger.info(f"[Hailo8] Exporting model to ONNX with nms=False, opset=11, batch=1...")
            await self.log_progress(model_id, "[Hailo8] Exportando modelo .pt a ONNX (Ultralytics)...")
            try:
                from ultralytics import YOLO
                model = YOLO(pt_path)
                onnx_path = model.export(format="onnx", imgsz=img_size, batch=1, nms=False, opset=11)
            except Exception as e:
                logger.exception(f"[Hailo8] ONNX export failed: {e}")
            ...
            if not dataset_key:
                logger.error("[Hailo8] Dataset key is required for calibration data generation")
            ...
            try:
                ...
                logger.info(f"[Hailo8] Downloading dataset {dataset_key} for calibration...")
                await self.log_progress(model_id, "[Hailo8] Descargando y preparando imágenes para calibración...")
                ...
                logger.info(f"[Hailo8] Generating calibration images from {len(selected_images)} frames...")
                ...
                logger.exception(f"[Hailo8] Calibration preparation failed: {e}")
            ...
            try:
                ...
                logger.info(f"[Hailo8] Creating Docker container: {' '.join(run_cmd)}")
                await self.log_progress(model_id, "[Hailo8] Creando contenedor Docker para compilador Hailo Model Zoo...")
                ...
                if proc.returncode != 0:
                    ...
                    logger.error(f"[Hailo8] Failed to run Docker container: {err_msg}")
                ...
                py_script = (
                    ...
                    f"'--yaml', '/local/workspace/hailo_model_zoo/hailo_model_zoo/cfg/networks/{yaml_name}', '--classes', '{num_classes}', '--hw-arch', 'hailo8']\n"
                ...
                logger.info(f"[Hailo8] Executing compile script in container...")
                await self.log_progress(model_id, "[Hailo8] Ejecutando compilación de red neuronal en contenedor Hailo...")
                ...
                if returncode != 0:
                    logger.error(f"[Hailo8] Compile failed inside container")
                ...
                logger.warning(f"[Hailo8] Compilation job for model {model_id} was cancelled.")
                raise
            except Exception as e:
                logger.exception(f"[Hailo8] Unexpected error during docker compilation: {e}")
                return CompilationResult(success=False, error=f"Docker compilation error: {e}")
            finally:
                # Clean up container
                logger.info(f"[Hailo8] Cleaning up Docker container {container_name}...")
                ...
                try:
                    ...
                    logger.warning(f"[Hailo8] Failed to clean up container: {e}")
            ...
            try:
                ...
                logger.info(f"[Hailo8] Uploading compiled model to MinIO: {compiled_key}...")
                await upload_bytes("compiled", compiled_key, hef_data)

                logger.info(f"[Hailo8] Compilation successful -> {compiled_key}")
                ...
                logger.exception(f"[Hailo8] Failed to upload compiled model to MinIO: {e}")
